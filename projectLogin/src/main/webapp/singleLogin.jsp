...
<body>
	<%
			...
			String str4 = "SELECT * FROM CustomerRep WHERE username =" + "'"+ user +"'" + " AND password=" + "'" + pw + "'";
			String str5 = "SELECT * FROM companies WHERE username =" + "'"+ user +"'" + " AND password=" + "'" + pw + "'";
			...
			else if(user.equals("admin") && pw.equals("admin#123")) {
				session.setAttribute("user", "admin");
				response.sendRedirect("AdminStaff.jsp");
			}
			else if (result5.next()){
				session.setAttribute("user", user);
				response.sendRedirect("companyPortal.jsp");
			}
			else
			{
				out.println("<script>alert('Incorrect username or password. Please try again.'); window.location.href = 'HelloWorld.jsp';</script>");
			}
